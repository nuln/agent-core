...
type CommandRegistry struct {
	mu        sync.RWMutex
	commands  map[string]*CustomCommand // from config or runtime add
...
// ClearSource removes all commands registered under a given source (e.g. "config").
...
// Remove deletes a command by name. Returns false if not found.
...
// SetAgentDirs sets the directories to scan for agent command files (*.md).
...
func (r *CommandRegistry) ListAll() []*CustomCommand {
	...
	result := make([]*CustomCommand, 0, len(r.commands))
	...
	for _, dir := range r.agentDirs {
		...
		for _, entry := range entries {
			...
			if seen[strings.ToLower(cmdName)] {
				continue
			}
			seen[strings.ToLower(cmdName)] = true
...
// placeholderRe matches {{1}}, {{2*}}, {{args}}, and variants with defaults.
