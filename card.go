...
// access-specific cards (Feishu Interactive Card, Telegram message, etc.)
// or degraded to plain text for accesses without card support.
type Card struct {
	Header   *CardHeader
...
// On Feishu this maps to div+extra; on other accesses it degrades to a text line.
...
// On Feishu this maps to select_static; on other accesses it degrades to text.
...
type CardButton struct {
	...
	Extra map[string]string // additional key-value pairs carried in the callback (access-specific)
}

// CardActionLayout controls how a CardActions row should be rendered by
// accesses with richer layout capabilities.
...
// on accesses that support richer layouts.
...
// RenderText converts the card to a plain-text representation for accesses
