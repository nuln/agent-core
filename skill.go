...
type SkillRegistry struct {
	mu    sync.RWMutex
	dirs  []string
	cache []*Skill // nil = not yet scanned
}
...
// Calling this invalidates the cache.
...
// Hyphens and underscores are treated as equivalent so that
// Telegram-sanitized names match original skill names.
...
// ListAll returns all discovered skills. Results are cached after the first scan.
func (r *SkillRegistry) ListAll() []*Skill {
	...
	if r.cache != nil { // double-check after acquiring write lock
		return r.cache
	}
